...
public class RobotContainer {
    ...
    private static final ShuffleboardTabManager m_shuffleboardTabManager = new ShuffleboardTabManager(m_swerve, m_conveyor, m_elevator, m_intake, m_pivot, m_shooter);
    ...
    private void configButtonBindings() {
                ...
                new InstantCommand(() -> currentState = SuperstructureState.AUTO_FIRE)
                    .andThen(
                        new ContinuousConditionalCommand(
                            ...
                            new PoseShooting(Controlboard.getTranslation(), () -> false, m_swerve, m_pivot, m_elevator, m_shooter, m_conveyor, m_intake, m_led), 
        ...
        new Trigger(m_conveyor.hasPiece(false))
            .and(Controlboard.intakeFromFloor().or(Controlboard.driverController_Command.rightStick()))
            .onTrue(
    ...
    public static boolean forwardPivotLimit(){
        ...
        boolean pivotAtMaxPivot = m_pivot.getPivotAngle().getDegrees() >= PivotConstants.FORWARD_SOFT_LIMIT.getDegrees();
        return elevatorAtMaxHeight && pivotAtMaxPivot;
    }
    
    public static boolean reversePivotLimit(){
        boolean elevatorAtMaxHeight = m_elevator.getElevatorHeight() >= ElevatorConstants.MAX_HEIGHT - ElevatorConstants.TARGET_THRESHOLD;
        boolean pivotAtMaxPivot = m_pivot.getPivotAngle().getDegrees() <= PivotConstants.REVERSE_SOFT_LIMIT.getDegrees();
        return elevatorAtMaxHeight && pivotAtMaxPivot;
    }
